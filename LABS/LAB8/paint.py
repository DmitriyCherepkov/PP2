        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1: # left click grows radius
                    radius = min(200, radius + 20)
                elif event.button == 3: # right click shrinks radius
                    radius = max(1, radius - 20)
...
def drawLineBetween(screen, index, start, end, width, color_mode, shape):
    ...
    for i in range(iterations):
        ...
        if shape == 'rectangle': pygame.draw.rect(screen, color, (x - width // 2, y - width // 2, width, width))
        else: pygame.draw.circle(screen, color, (x, y), width)
